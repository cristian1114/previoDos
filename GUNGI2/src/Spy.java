 ...
 *  Spy Class
 *  <p>
 *  A Spy is a specific piece of the game
 */
public class Spy extends Pawn{
     ...
     */
    public Spy(int xx, int yy, Vector2D rc, int pl_color){
		super(xx,yy,rc, pl_color);
        valueIA = 3;
	}
	public Spy(int xx, int yy, int rr, int cc, int pl_color){
    	...
    	switch(this.level){
    		case 0:{
    			...
    			for (BoardPanel e:elements){
    				if (e.getRow() == fila_actual+dif*2 && e.getColumn()==columna_actual+1){
    				...
    				else if (e.getRow() == fila_actual+dif*2 && e.getColumn()==columna_actual-1){
                        int ss = e.getTierSize();
                        switch(ss){
                            case 0: posibles.add(e); break;
                            case 1: 
